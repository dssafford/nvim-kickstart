...
require('lazy').setup({
  ...
  dependencies = {
    'nvim-lua/plenary.nvim',
    { -- Optional native sorter for performance
      'nvim-telescope/telescope-fzf-native.nvim',
      build = 'make',
      cond = function()
        return vim.fn.executable 'make' == 1
  ...
  config = function()
    require('telescope').setup {
      defaults = {
        ...
        file_ignore_patterns = { 'node_modules', '.git/', 'venv/', '%.log' },
        winblend = 10,
        prompt_prefix = '🔍 ',
        selection_caret = ' ',
    ...
    -- Fast fuzzy search in current buffer
    vim.keymap.set('n', '<leader>/', function()
      builtin.current_buffer_fuzzy_find(require('telescope.themes').get_dropdown {
        winblend = 10,
        previewer = false,
    ...
    -- 🔍 New: find all files including hidden + ignored
    vim.keymap.set('n', '<leader>fa', function()
      builtin.find_files { hidden = true, no_ignore = true }
    end, { desc = '[F]ind [A]ll files (incl. hidden)' })

    -- 🔍 New: find buffers (shortcut)
    vim.keymap.set('n', '<leader>fb', builtin.buffers, { desc = '[F]ind [B]uffers' })

    -- 🔍 New: help tags again (duplicate of <leader>sh for flexibility)
...
-- open tree on left on start every time
