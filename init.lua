...
-- print 'eat shit and bark at the fucking moon doug'
-- 💡 Disable Neovim's built-in snippet engine BEFORE cmp loads
vim.g.mapleader = ' '
vim.g.maplocalleader = ' '

-- 🔥 Hard-disable vim.snippet (even if a plugin resets it)
vim.schedule(function()
  vim.snippet = nil

  -- Helper to safely delete keymaps
  local function safe_del(mode, lhs)
    pcall(vim.api.nvim_del_keymap, mode, lhs)
  end

  -- Remove potential conflicting keymaps
  safe_del("i", "<Tab>")
  safe_del("s", "<Tab>")
  safe_del("i", "<S-Tab>")
  safe_del("s", "<S-Tab>")
...
local todo_float = require 'kickstart.plugins.todofloat'
todo_float.setup {
  target_file = '~/notes/todo.md',
}

-- This allows `nvr` to communicate with your current Neovim session
...
vim.api.nvim_create_autocmd('BufEnter', {
  pattern = 'oil://*',
  callback = function()
    vim.opt_local.winbar = '󰏖 Oil Explorer'
  end,
})
vim.api.nvim_create_autocmd("User", {
  ...
  once = true,
  callback = function()
    -- 🔥 Force-disable Neovim's built-in snippet engine if it was re-enabled
    vim.snippet = nil

    -- 🔒 Force delete tab/enter overrides from any plugin
    ...
    vim.notify("🔥 Cleared all snippet overrides", vim.log.levels.INFO)
  end,
})
